...
} = require('./logic/browser/stats');
const {
  decodeEscapedUrl,
  normalizeQuestLivePayload,
  pollQuestLiveRaw
} = require('./logic/browser/quest-live');
...
const persistedWindowConfig = loadPersistedWindowConfig(WINDOW_CONFIG_PATH);
...
async function startBrowser(socket, sessionStats) {
  ...
  browser = await launchWithRetries({
    puppeteer,
    socket,
    cfg: normalized,
  ...
  const collectStats = async () => {
    ...
    try {
      ...
      if (shouldPollQuestLive) {
        ...
        const questLiveRaw = await pollQuestLiveRaw(page, questGetEndpointCache);
        ...
        if (questLiveRaw && questLiveRaw.payload) {
          ...
          } else if (!didLogQuestLiveReady) {
            didLogQuestLiveReady = true;
            const qp = normalizedQuest.quest_points ?? '?';
            const maxQp = normalizedQuest.max_quest_points ?? '?';
            socket?.emit('bot-log', `Quest live reader active (no quest rows yet, points ${qp}/${maxQp})`);
          }
        } else {
          if (Date.now() - lastQuestLiveMissLogAt > 60000) {
            lastQuestLiveMissLogAt = Date.now();
            socket?.emit('bot-log', 'Quest live reader: no payload this cycle');
          }
        }
      }
    } catch {
...
function setBrowserWindowConfig(cfg = {}) {
  ...
  persistWindowConfig(WINDOW_CONFIG_PATH, windowConfig);
  return windowConfig;
}
